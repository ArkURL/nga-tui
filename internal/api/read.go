...
func (c *Client) GetThreadContent(tid int, page int) (*ThreadContentResult, error) {
	...
	res := &ThreadContentResult{}
	err := c.fetchJSON("/read.php", params, func(body []byte) error {
		data, err := parseData(body)
		if err != nil {
			return err
		}
		res.Page = intFromRaw(data["__PAGE"])
		res.Rows = intFromRaw(data["__ROWS"])
		if res.Page == 0 {
			res.Page = page
		}
		...
		if rawR, ok := data["__R"]; ok {
			if err := json.Unmarshal(rawR, &res.Replies); err != nil {
				return fmt.Errorf("解析楼层 __R: %w", err)
			}
		}
		...
		if rawU, ok := data["__U"]; ok {
			...
			if err := json.Unmarshal(rawU, &umap); err != nil {
				return fmt.Errorf("解析用户 __U: %w", err)
			}
